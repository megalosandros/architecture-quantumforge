...
|---------|-----|------------|--------------|-------|--------|
| **A. Полностью локальный (рекомендуемый)** | Llama 3 8B (GGUF, 4-bit, llama.cpp) | bge-m3 | ChromaDB (persistent) | Полная безопасность, нулевые OpEx после запуска, SOC2-compliant | Требуется GPU-сервер, ~$20k CapEx |
...
Для эмбеддинга выбрана модель [BAAI/bge-m3](https://huggingface.co/BAAI/bge-m3), в качестве векторной базы - ChromaDB.  
Генерация индекса выполнялась в среднем ~ЗАПОЛНИТЬ секунд.  
